...
def create_user(user_credentials: UserSchema) -> dict:
    try:
        ...
        pw = user_credentials.password
        role_str = user_credentials.role.upper()
...
def login(user_credentials: Login) -> str:
    user = authenticate_user(user_credentials)
    if not user:
        return None
    token_expires = timedelta(minutes=30)
    token = create_access_token({"sub": user.email, "role": user.role.name}, token_expires)
    return token

def authenticate_user(user_credentials: Login) -> UserSchema:
...
def check_token(token: str):
    try:
        ...
        return {"success": True, "body": {"user_id": user.id, "user_email": user.email, "user_role": user.role.name}, "status_code": 200}
